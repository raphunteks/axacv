...
(function() {
    ...
    // =====================================================================
    // 2. CHIEF MASTER ENTERPRISE UI/UX: SPLIT MAGNETIC HUD CURSOR
    // =====================================================================
    
    // Hanya inisiasi kursor jika perangkat menggunakan Mouse/Pointer presisi (Bukan Touchscreen)
    if (window.matchMedia("(hover: hover) and (pointer: fine)").matches) {
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            cursorStyles.innerHTML = `
                body, a, button, input, textarea, select, label, svg, img, .link-hover, .btn-pulse, .social-lift, .admin-tab, .bg-swatch, .color-swatch, .theme-btn { 
                ...
                .hud-bracket {
                    ...
                    border-style: solid; opacity: 0.8;
                    filter: drop-shadow(0 0 6px var(--color-primary, #00E5FF));
                    transition: border-width 0.3s ease, border-color 0.3s ease, filter 0.3s ease, opacity 0.3s ease;
                }
                
                .magnetic-active .hud-bracket {
                    border-color: var(--color-primary, #00E5FF); opacity: 1;
            ...
            // Elemen apa saja yang akan mengaktifkan kunci magnetik (Bisa dtambah class baru di sini)
            const magneticSelector = '.premium-card, .admin-item-card, .stat-breathe, a, button, input, textarea, select, label, svg, img, .link-hover, .btn-pulse, .social-lift, .theme-btn, .color-swatch, .bg-swatch, .admin-tab';
            ...
            document.addEventListener('mousedown', () => { if(hasMoved) scale = 0.85; });
            document.addEventListener('mouseup', () => { if(hasMoved) scale = 1; });

            // Event Delegation untuk mendeteksi elemen secara dinamis (tanpa perlu observer)
            ...
            // Loop Frame GPU Animation
            const renderHUD = () => {
                if (hasMoved) {
                    // 1. FREE TRACKING INNER DOT (Dot selalu ikut mouse)
                    ...
                    if (isHovering && hoveredEl && hoveredEl.isConnected) {
                        ...
                        // 4. ROTATING IDLE STATE (Berputar saat tidak kena box)
                        rot += 1.5; 
                    ...
                    // 5. ANIMASI LERP HALUS UNTUK BRACKET
